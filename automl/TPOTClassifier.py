...
#		{ "name": "verbosity" , "field": "verbosity", "kind": "integer", "min": 2, "max": 100, "required": true, "default": 2 },
...
def TPOTClassifier(ATM):
	X = ATM.inputs["X"]
	y = ATM.inputs["y"]
	tpot = TPOTClassifier(generations=ATM.props["generations"], population_size=ATM.props["population_size"], verbosity=ATM.props["verbosity"], random_state=ATM.props["random_state"])
	tpot.fit(X, y)
	ATM.report({ 'name': "stats", 'stats': { 'score': tpot.score(payload.testing_data, y_testing_targets) } })
